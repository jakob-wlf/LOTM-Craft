...
public class Recording extends Ability {

    private final Set<Beyonder> onCooldown = new HashSet<>();

    public Recording(Pathway pathway, int sequence, AbilityType abilityType, String name, Material material, String description, String id) {
    ...
    public void useAbility(Beyonder beyonder) {
        ...
        Bukkit.getScheduler().runTaskLater(plugin, () -> onCooldown.remove(beyonder), 30);
        ...
        Marker abilityMarker = entity.getNearbyEntities(10, 10, 10).stream().filter(e -> e.getType() == EntityType.MARKER).map(e -> (Marker) e).filter(marker -> {
        ...
        Ability ability = LOTM.getInstance().getAbilitiesBeingUsed().get(abilityMarker.getUniqueId());
        if(ability == null)
            return;

        ability = ability.copy(AbilityType.RECORDED);
        if(ability == null)
            return;
